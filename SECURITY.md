...
This marketplace therefore cannot rely on the installer to refuse an unverified
plugin. Enforcement lives at four points instead:

1. **Catalog admission** — a plugin SHA enters `marketplace.json` only after its
   attestations verify in CI (fail-closed at merge).
2. **SHA-pinned catalog** — external plugin sources are pinned to a 40-char `sha`
   (native), so cataloged content is immutable.
3. **Cosign-signed catalog** — the `marketplace.json` blob is keyless-signed, so
   a consumer can prove the catalog they fetched is the one this repo published.
...
> **Every gate is risk-reducing, not risk-eliminating.** A passing verification
> proves a gate *ran and recorded a verdict* bound to the subject digest. It does
> not certify the plugin is benign.

---

## Verify a plugin release

Each plugin release is a tarball signed with GitHub's Sigstore-backed (keyless,
OIDC) attestation infrastructure. Anyone can re-verify from a clean workstation —
...
REPO="attested-delivery/claude-code-plugins"
...
The artifact-characterizing gates are each signed by the central attestation seam
(`reusable-attest-scan.yml`). Under SLSA Build L3 the Fulcio signer identity is
that central workflow, so `--owner`/`--repo` alone is **not** sufficient — pin
`--signer-workflow`, one predicate per command:

```bash
for pt in shellcheck semgrep secrets manifest sast sca iac-license; do
  gh attestation verify "$TARBALL" --owner attested-delivery \
...
The `marketplace.json` catalog is a blob (not an OCI image), signed with **cosign
keyless** (Sigstore Fulcio/Rekor). Verify the downloaded catalog against its
detached bundle:
...
```
Loaded digest sha256:abc123... for file://attested-reference-0.1.0.tar.gz
...
| SCA | `.../attestations/sca/v1` | seam | OSV ran over MCP dependencies and recorded a verdict |
...
- Every external plugin source in `marketplace.json` is pinned to a 40-char
  `sha`; the **manifest-review** gate fails closed if any is not.
- The release pipeline is fail-closed: every attestation must verify before a
  plugin SHA is admitted to the catalog and before a release publishes. There is
  no path from build to publish that bypasses verification.
